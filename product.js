...
function renderProduct(p) {
  ...
  const descriptionHtml =
    cleanDescription(p.description || p.description_ru || "") || '<div class="spec-line">Описание уточняется.</div>';
...
function readProductIdFromPage() {
  ...
  if (!id) {
    try {
      id = (sessionStorage.getItem("zwillon_pending_product_id") || "").trim();
...
async function init() {
  ...
  const id = readProductIdFromPage();
  ...
  if (!id) {
    showProductError(
      "ID НЕ ПЕРЕДАН",
      'Откройте карточку из <a href="catalog.html" class="text-accent underline hover:text-white">каталога</a> или перейдите по ссылке вида <code class="text-accent">/product.html?id=…</code>.'
    );
    return;
  }
